#!/usr/bin/env bash
# LiveRecorder build script
#   ./build.sh          → build release APK, bump version, commit to repo
#   ./build.sh --no-bump → build without incrementing version
#
# Archived APK path: apk/liverecorder-<versionName>.apk
# Local archive keeps only the newest 2 APKs (pruned below).
#
# The CI workflow (build.yml) also creates a matching GitHub Release
# (tag vX.Y.Z) with that APK as an asset. The CI keeps only the newest 2
# Releases/tags on GitHub as well (old ones are pruned in build.yml).
...
APK_ARCHIVE="apk"
APK_NAME="liverecorder-release.apk"
...
mkdir -p "$APK_ARCHIVE"
NEW_APK="$APK_ARCHIVE/liverecorder-$(grep '^versionName=' "$VERSION_FILE" | cut -d= -f2).apk"
...
# (files matching liverecorder-<version>.apk, sorted by version descending)
mapfile -t OLD_APKS < <(ls -1 "$APK_ARCHIVE"/liverecorder-*.apk 2>/dev/null | sort -rV)
...
ls -lh "$APK_ARCHIVE"/liverecorder-*.apk 2>/dev/null | awk '{print "  → "$NF}'
